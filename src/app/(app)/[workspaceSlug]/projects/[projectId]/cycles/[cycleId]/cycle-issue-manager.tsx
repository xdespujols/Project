...
export function CycleIssueManager({ cycleId, assignedIds, unassignedIssues }: Props) {
  const router = useRouter();
  const [loading, setLoading] = useState<string | null>(null);

  async function addIssue(issueId: string) {
    setLoading(issueId);
    await fetch(`/api/v1/projects/unknown/cycles/${cycleId}/issues`, {
      method: 'POST',
  ...
  return (
    <div>
      ...
      {unassignedIssues.length === 0 ? (
        <p className="text-xs text-gray-400">All issues are in this cycle.</p>
      ) : (
